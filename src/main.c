...
static declare_choice_callback_g(month)
{
  const static struct StoryPage story[] = {
    { .title = "Dim morning",
     .text
      = "You wander into Independence as the weak sun breaks the horizon, horse in toe. The journey has here was tiresome, but "
...
static declare_choice_callback(month_advice)
{
  showInfoDialog(
      ...
      "early, there won't be any grass for your oxen to eat. If you leave too late, you may not get to Oregon before winter "
...
static declare_choice_callback(role_learn)
{
  showLongInfoDialog(
      ...
      "carpenter or a farmer.\n\nHowever, the harder you have to try, the more points you deserve! Therefore, the farmer earns "
...
static declare_choice_callback(main_start) { showRole(); }
