...
#define BUFFER_LEN 1024
	...
	PaError paError = Pa_OpenStream(
		...
		sfInfo.samplerate, paFramesPerBufferUnspecified, paNoFlag,
		NULL, NULL);
	if (paError || !stream)
		goto error;
	if (paError = Pa_StartStream(stream))
		goto error;
	...
	{
		if (paError = Pa_WriteStream(stream, data, readCount))
			goto error;
		memset(data, 0, sizeof(data));
	}
	if (paError = Pa_CloseStream(stream))
		goto error;
	return 0;
